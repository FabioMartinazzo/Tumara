...
var Logger = {
	writeToFile : function(req, msg, fileKind) {
			...
			parser.parseString(data, function (err, fileObj) {
				...
				fileObj.xml.config.forEach(function(node) {
					...
					var msgNoLN = msg + ((req != undefined) ? (" - Follower : " + 
							 utilities.getFollower(req.body) + utilities.getAccessIp(req, true)) : '') +  							 	 utilities.getCurrentDate(true, true)
					msg = msgNoLN + utilities.lineBreak()

					fs.appendFile(path + fileName, msg, function(err) {
						if (err)
...
module.exports = {
	saveAccessLog : function(req, msg) {
		Logger.writeToFile(req, msg, 0)
	},
	saveErrorLog : function(req, msg) {
		Logger.writeToFile(req, msg, 1)
	}
}
